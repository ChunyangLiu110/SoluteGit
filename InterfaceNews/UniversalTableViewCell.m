...
- (void)setModel:(RstModel *)model {
    ...
    if (model.author_list.count >= 1 ) {
        NSArray *array = model.author_list;
        Author_listModel *authorModel = array[0];
        _nameLabel.text = authorModel.name;
    }
    _updateTimeLabel.text = [self transformTime:model.update_time];
...
- (void)layoutSubviews {
    ...
    _titleLabel.frame = CGRectMake(leftPadding, CGRectGetMaxY(_imageView.frame)+padding, size.width-2*leftPadding, 43);
